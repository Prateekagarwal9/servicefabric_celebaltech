...
module.exports = generators.Base.extend({
    ...
    */
    ...
    initializing: function () {
        ...
        this.registryName = this.props.registryName;
        this.registryusername = this.props.registryusername;
        this.registrypassword = this.props.registrypassword;
    }, // initializing()
    /**
   ...
   _assert: function(condition, message) {
    ...
    writing: {
        application: function() {
            ...
            var regusername = this.registryusername;
            var regpassword = this.registrypassword;
                    ...
                    parser.parseString(data, function (err, result) {
                        ...
                        if (portMapHost != "" && portMapContainer != ""){
                            result['ApplicationManifest']['ServiceManifestImport'][result['ApplicationManifest']['ServiceManifestImport'].length] =
                            {
                                ...
                                "Policies":[{"ContainerHostPolicies":[{"$":{"CodePackageRef":"Code"},"RepositoryCredentials":[{"$":{"AccountName": regusername, "Password": regpassword}}],"PortBinding":[{"$":{"ContainerPort": portMapContainer, "EndpointRef": serviceEndPointName}}]}]}]
                            }
                          //  result['ApplicationManifest']['ServiceManifestImport'][result['ApplicationManifest']['ServiceManifestImport'].length] =
                            // {
                            //     "ServiceManifestRef":[{"$":{"ServiceManifestName":servicePkgName, "ServiceManifestVersion":"1.0.0"}}],
                            //     "Policies":[{"ContainerHostPolicies":[{"$":{"CodePackageRef":"Code"},"RepositoryCredentials":[{"$":{"AccountName": regusername, "Password": regpassword}}]}]}]
                            // }
        ...
        service: function() {
            ...
            var finalimagename = this.registryName + '/' + this.props.imageName;
            ...
            if (!this.isAddNewService) {
                ...
                this.fs.copyTpl(
                this.templatePath('PublishProfiles/cloud.xml'),
                this.destinationPath(this.destinationRoot() , 'PublishProfiles/cloud.xml'));
                this.fs.copyTpl(
                this.templatePath('PublishProfiles/Local.1Node.xml'),
                this.destinationPath(this.destinationRoot() , 'PublishProfiles/Local.1Node.xml'));
                this.fs.copyTpl(
                this.templatePath('PublishProfiles/Local.5Node.xml'),
                this.destinationPath(this.destinationRoot() , 'PublishProfiles/Local.5Node.xml'));
                this.fs.copyTpl(
                this.templatePath('ApplicationParameters/Cloud.xml'),
                this.destinationPath(this.destinationRoot() , 'ApplicationParameters/Cloud.xml'),
                {
                  appparameterName : "fabric:/"+this.projName
                });

                this.fs.copyTpl(
                this.templatePath('ApplicationParameters/Local.1Node.xml'),
                this.destinationPath(this.destinationRoot() , 'ApplicationParameters/Local.1Node.xml'));
                this.fs.copyTpl(
                this.templatePath('ApplicationParameters/Local.5Node.xml'),
                this.destinationPath(this.destinationRoot() , 'ApplicationParameters/Local.5Node.xml'));
                  this.fs.copyTpl(
